...
impl Monkey {
    pub fn inspect(&mut self, i: usize) -> (usize, usize) {
        self.inspections += 1;
        self.items[i] = self.operation.evaluate(self.items[i]) / 3;
...
impl FromStr for Monkey {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let l_item = ll.nth(1).unwrap();
        let l_operation = ll.next().unwrap();
        let l_test = ll.next().unwrap();
        let l_true = ll.next().unwrap();
        let l_false = ll.next().unwrap();
        ...
        let operator = l_operation[21..22].parse().unwrap();
        let operand = l_operation[22..].trim().parse().unwrap();
        let divisor = l_test[19..].trim().parse().unwrap();

        let targets = (
            l_true[25..].trim().parse().unwrap(),
            l_false[25..].trim().parse().unwrap(),
        );
