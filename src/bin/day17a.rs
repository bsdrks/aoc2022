...
pub struct Room {
    cache: HashMap<(RockType, usize, Vec<i64>), i64>,
...
impl Room {
    ...
    fn relief(&self) -> Vec<i64> {
        (0..7)
            .map(|x| {
                (0..=self.top)
                    .rev()
                    .find(|&y| self.solid.contains(&point!(x, y)))
                    .unwrap_or(-1)
            })
            .collect()
    }

    fn free(&self, rock: RockType, offset: Point) -> bool {
    ...
    pub fn drop(&mut self) -> i64 {
        loop {
            let i_jet = self.turn % self.jets.len();
            let rock = RockType::index(self.dropped);

            match self.jets[i_jet] {
            ...
            if self.free(rock, point!(0, -1)) {
                if let Some(previous) = self.cache.insert((rock, i_jet, self.relief()), self.top) {
                    return previous;
                }
