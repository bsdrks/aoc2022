    ...
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        match (self, other) {
            ...
            (Self::List(_), Self::Number(b)) => {
                self.partial_cmp(&Self::List(vec![Self::Number(*b)]))
            }
        }
    }
}
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        while let Some(c) = chars.next() {
            match c {
                ...
                _ => {
                    if let Self::List(list) = &mut packet {
                        list.push(Self::Number(c.to_digit(10).unwrap() as usize));
                    }
                }
            }
        }
...
fn solve(s: &str) -> usize {
    ...
    for s in pairs {
        let mut p = s.lines().map(|s| s.parse::<Packet>().unwrap());
